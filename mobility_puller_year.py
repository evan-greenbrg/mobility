    ...
    with fiona.open(polygon_path, layer=polygon_name) as layer:
        for feature in layer:
            ...
            for year in years:
                ...
                river_root = out_root.format(year)
...
def main(polygon_path, out_root, keep, river, year_range):
    ...
    yearly_mobility = get_mobility_yearly(
        images,
        clean_channel_belts,
        year_range
    )

    yearly_mobility['dt'] = pandas.to_datetime(
        yearly_mobility['year'],
        format='%Y'
    )
...
#    yearly_mobility, monthly_mobility = main(polygon_path, out_root)
    ...
    yearly_mobility.to_csv(
        os.path.join(args.out, f'{args.river}_yearly_mobility.csv')
    )
#
